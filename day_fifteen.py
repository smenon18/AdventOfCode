...
def main() -> None:
    ...
    limit = 2000000
    beacons = set()  # keep set of beacons and signals to remove from other set at end
    beaconless = set()
    sensors = {}
    ...
    for s_point, dist in sensors.items():
        sx, sy = s_point  # create ranges with this that mimicks the data passed into set union from p1
            ...
            for j in [sy + (dist + 1 - abs(i - sx)), sy - (dist + 1 - abs(i - sx))]:
                if j in range(max_xy + 1):
                    for s_p2, d2 in sensors.items():
                        ...
                        print('Part B:')
                        print(i * max_xy + j)
                        return  # would be better to make this look function and return printed line above
